/*
 * @Author: Ishar-zdl 
 * @Date: 2023-09-28 17:43:28 
 * @Last Modified by:   Ishar-zdl 
 * @Last Modified time: 2023-09-28 17:43:28 
 */
#include<bits/stdc++.h>
using namespace std;
const int N=5e6+5;
...
void dfs(int x,int die,int q){
    ...
    for(int i=1;i<=k;i++)f[x][i]=f[f[x][i-1]][i-1],ma[x][i]=max(ma[x][i-1],ma[f[x][i-1]][i-1]);
    for(int i=head[x];i;i=edge[i].next)
        if(edge[i].v!=die)dfs(edge[i].v,x,edge[i].w);
}
inline void lca(int a,int b){
    // cout<<a<<b<<endl;
    if(get(a)^get(b)){printf("impossible\n");return;}
    if(d[a]<d[b])swap(a,b);int ans=-1e9;
    while(d[a]>d[b]){
        // cout<<ma[2][0]<<endl;
        // cout<<a<<' '<<b<<' ';
        // cout<<d[a]-d[b]<<' ';
        ...
        ans=max(ans,ma[a][__lg(d[a]-d[b])]),a=f[a][__lg(d[a]-d[b])];
    }
    if(a==b){write(ans);putchar('\n');return;}
    // cout<<"fdskl"<<endl;
    for(int i=__lg(d[b]);i>=0;--i)
        if(f[a][i]!=f[b][i])ans=max(ans,max(ma[a][i],ma[b][i])),a=f[a][i],b=f[b][i];
